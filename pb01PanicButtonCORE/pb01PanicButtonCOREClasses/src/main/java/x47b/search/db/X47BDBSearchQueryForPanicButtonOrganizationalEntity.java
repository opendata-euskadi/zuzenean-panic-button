...
/////////////////////////////////////////////////////////////////////////////////////////
	static class X47BDBSearchQueryForPanicButtonOrganizationalEntityToJPQLTranslator
	    ...
	    protected String _composeWhereJpqlPredicates(final X47BSearchFilterForPanicButtonOrganizationalEntity filter,
			...
			// (the same applies to divisions, services or locations)
			if (filter.getOrganizationOid() != null) {
				filteredDBEntities.remove(X47BDBEntityForOrganization.class.getSimpleName());
			}
			if (filter.getOrgDivisionOid() != null) {
				filteredDBEntities.remove(X47BDBEntityForOrganization.class.getSimpleName());
				filteredDBEntities.remove(X47BDBEntityForOrgDivision.class.getSimpleName());
			}
			if (filter.getOrgDivisionServiceOid() != null) {
				filteredDBEntities.remove(X47BDBEntityForOrganization.class.getSimpleName());
				filteredDBEntities.remove(X47BDBEntityForOrgDivision.class.getSimpleName());
				filteredDBEntities.remove(X47BDBEntityForOrgDivisionService.class.getSimpleName());
			}
			if (filter.getLocationOid() != null) {
				filteredDBEntities.remove(X47BDBEntityForOrganization.class.getSimpleName());
				filteredDBEntities.remove(X47BDBEntityForOrgDivisionServiceLocation.class.getSimpleName());
			}
			String entitiesToSearch = CollectionUtils.of(filteredDBEntities)
											  		 .toStringCommaSeparated();
			log.info("Filtering entities of types: {}",entitiesToSearch);
			...
			String nameDBColName = _nameDBColum(filter);
			
			// [1] - Filter by text
			if (filter.hasTextFilter() && Strings.isNOTNullOrEmpty(filter.getText())) {
				boolean fullTextSearchSupported = _dbModuleConfig.isFullTextSearchSupported(_entityManager);
				...
				if (fullTextSearchSupported) {
					...
					DBVendor dbVendor = _dbModuleConfig.getDbSpec().getVendor();
	...
	//		}
			...
			if (filter.getLocationOid() != null)  	 jpqlWhere.append("AND entity._locationOid = :locOid ");
		...
		protected String _composeJpqlOrderByClause(final X47BSearchFilterForPanicButtonOrganizationalEntity filter,
			...
			String nameDBCol = _nameDBColum(filter);
			return "entity._hierarchyLevel ASC," +	// return ordered results: organizations, locations and work places
										 nameDBCol + " ASC," +
								         "entity._oid ASC";
		}
		@Override
	    public void setJPAQueryParameters(final X47BSearchFilterForPanicButtonOrganizationalEntity filter,
			...
			if (filter.getLocationOid() != null) 	 qry.setParameter("locOid",filter.getLocationOid().asString());
		}
		private String _nameDBColum(final X47BSearchFilterForPanicButtonOrganizationalEntity filter) {
			...
			String nameDBColName = filteringLang.is(Language.SPANISH) ? "entity._nameSpanish"
																      : "entity._nameBasque";
