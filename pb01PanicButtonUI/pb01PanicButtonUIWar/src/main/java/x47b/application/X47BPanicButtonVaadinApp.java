...
/////////////////////////////////////////////////////////////////////////////////////////
	@Getter private ResourceBundle _bundle;
			...
			//N�mero de registros por p�gina de la tabla
			private int _itemsPerPage = 10;
			//N�mero de resultados totales
			private long _totalItemsCount = 0;
			//�ndice para pintar las filas de la tabla
...
/////////////////////////////////////////////////////////////////////////////////////////
	...
	protected void init(final VaadinRequest request) {
		...
		//Si el combo no est� vacio mostramos las entidades de la primera organizaci�n
...
/////////////////////////////////////////////////////////////////////////////////////////
	...
	private void _buildPagedFilterTable() {
		...
		_pagedFilterTable.addGeneratedColumn(ALARM_COLUMN,
			...
			public Object generateCell(final CustomTable source,
				...
				if (nalarms > 0) {
					...
					//Comprueba si tiene hay alarmas en la �ltima media hora
					...
					//Evento click del n�mero de alarmas en la tabla
					edit.addClickListener(new ClickListener() {
												...
												public void buttonClick(ClickEvent event) {
													...
													} else if (resultItem.getModelObjectType().equals(X47BOrgDivisionServiceLocation.class)) {
														alarmCaption = resultItem.getLocation().getName();
													}
		...
		_pagedFilterTable.addGeneratedColumn(LOCATION_COLUMN,
			...
			public Object generateCell(CustomTable source, Object itemId, Object columnId) {
				...
				//Evento click de la LOCALIZACI�N en la tabla.
				//Edita la localizaci�n
			...
			public Object generateCell(CustomTable source, Object itemId,
				...
				if (resultItem.getModelObjectType().equals(X47BWorkPlace.class)) {
					...
					List<String> listIDs = Splitter.on('/')
												   .splitToList(resultItem.getHierarchyPath().asAbsoluteString());
	...
	/**
	 * Cabecera (Logo, barra de Organizaci�n, Cambio de idioma
	 * @return
	 */
	...
	private Component _buildOrganizationBarButtons() {
		...
		//Selecciona la primera organizaci�n del combo
	...
	private Component _buildOrganizationButtons() {
		//Bot�n para EDITAR ORGANIZACI�N
				...
				new Button.ClickListener() {
					...
					public void buttonClick(final ClickEvent event) {
						if ( _cboOrganizations.getValue() != null) {
							...
							w.setCaption(Strings.customized(_bundle.getString("x47b.msg.location.window.caption"), "\""+org.getName().getIn(_lang)+"\""));
							w.addCloseListener(new CloseListener() {
													@Override
													public void windowClose(final CloseEvent e) {
														_closeEntityUpdateWindowEventActions(w);
	...
	private Component _buildLocationButtons() {
		//Bot�n para EDITAR LOCALIZACION
	...
	/**
	 * Compone la ventana de edici�n de las entidades
	 * @param x
	 * @return
	 */
	...
	/**
	 * Acciones a realizar despu�s de cerrar la ventana de mantenimiento de entidades
	...
	private void _closeEntityUpdateWindowEventActions(final X47BEntityWindow w) {
		if ( w.getData() instanceof X47BOrgDivisionServiceLocation){
			//La ventana devuleve actualizaci�n de Localizaci�n o lugar de trabajo
			if (w.getSelectedBtn() == w.getSaveBtn() || w.getSelectedBtn() == w.getRemoveBtn()) {
				...
				//Se ha pulsado guardar, Hay que a�adir o modificar la organizaci�n al combo
				...
				if (w.isNewEntity()) {
					X47BSummarizedOrgDivisionServiceLocation sumLoc = new X47BSummarizedOrgDivisionServiceLocation().withOid(loc.getOid())
																						.withId(loc.getId())
																						.named(loc.getName().getIn(_lang).orNull());
					...
					if (_cboLocations.getItem(_cboLocations.getValue()) != null) {
						...
						sumLoc.named(loc.getName().getIn(_lang).orNull());
			...
			//La ventana devuleve actualizaci�n de Localizaci�n o puesto de trabajo
			if (w.getSelectedBtn() == w.getSaveBtn() || w.getSelectedBtn() == w.getRemoveBtn()) {

				// Se ha pulsado guardar, Hay que a�adir o modificar la organizaci�n al combo
				X47BWorkPlace workPlace = (X47BWorkPlace)w.getData();

				log.debug("----------------- NAME ES:  "+workPlace.getName().getIn(_lang));

				if (w.isNewEntity()) {
					X47BSummarizedWorkPlace sumWorkPlace = new X47BSummarizedWorkPlace().withOid(workPlace.getOid())
																				  		.withId(workPlace.getId())
																				  		.named(workPlace.getName().getIn(_lang).orNull());
					...
					if (_cboWorkPlaces.getItem(_cboWorkPlaces.getValue()) != null) {
						...
						sumWorkPlace.named(workPlace.getName().getIn(_lang).orNull());
			...
			if (w.getSelectedBtn() == w.getSaveBtn()) {
				...
				if (w.isNewEntity()) {
					X47BSummarizedOrganization sumOrg = new X47BSummarizedOrganization().withOid(org.getOid())
																						.withId(org.getId())
																						.named(org.getName().getIn(_lang).orNull());
					...
					sumOrg.named(org.getName().getIn(_lang).orNull());
	...
	private void _reloadTable(boolean initDatasource) {
		...
		if (_cboOrganizations.getValue() != null) {
	         ...
	         *************************************/
			log.debug("/*************************************");
			log.debug("EJECUTA B�SQUEDA");
			log.debug("*************************************/");
			log.debug("Primera a fila a devolver		:"+(_currentPage*_itemsPerPage-_itemsPerPage));
			log.debug("N�mero de elementos por p�gina	:"+_itemsPerPage);
			log.debug("Total elementos b�squeda			:"+_totalItemsCount);
			log.debug("*************************************/");
			...
			SearchResults<X47BSearchFilterForPanicButtonOrganizationalEntity,
			 ...
			 /*************************************
	         * Establece elementos de paginaci�n
	         *************************************/
	...
	/**
	 * Configura loas literales de la paginaci�n de la tabla
	 * @return
	 */
	private void _composePagedFilterControlConfig() {
		...
		//Evento de cambio de elementos por p�gina
		...
		//Evento de cambio de n�mero de p�gina
