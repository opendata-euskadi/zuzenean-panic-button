...
@Accessors(prefix="_")
public abstract class X47BEntityObjectBase<O extends X47BPersistableObjectOID,ID extends X47BModelObjectID<O>,
										   SELF_TYPE extends X47BEntityObjectBase<O,ID,SELF_TYPE>>
	          extends PersistableModelObjectBase<O,SELF_TYPE>
	  	   implements X47BEntityObject<O,ID> {
	...
	public Summarizable asSummarizable() {
		Summary sum = SummaryBuilder.languageInDependent()
									.create("{} object with id={}",
										    X47BEntityObjectBase.this.getClass(),_id.asString());
